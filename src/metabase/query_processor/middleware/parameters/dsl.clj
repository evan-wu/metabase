(ns metabase.query-processor.middleware.parameters.dsl
...
(s/defn ^:private honeysql->replacement-snippet-info :- ParamSnippetInfo
  ...
  [x]
  (let [[snippet & args] (hsql/format x, :quoting (constantly :ansi))]
    {:replacement-snippet     (str x)
...
(s/defn ^:private dsl->params-snippets-info :- [ParamSnippetInfo]
...
(s/defn ^:private add-replacement-snippet-info :- [ParamSnippetInfo]
  ...
  (for [snippet-info params-snippets-info]
    (handle-optional-snippet
      (merge snippet-info
        (s/validate ParamSnippetInfo (let [value (snippet-value snippet-info param-key->value)]
                                       ;(println (str "snippet-value: " value))
                                       ;(println (str "type: " (type value)))
...
(s/defn ^:private expand-query-params
  [{dsl :query, :as native}, param-key->value :- ParamValues]
  (merge native (when-let [param-snippets-info (seq (add-replacement-snippet-info (dsl->params-snippets-info dsl) param-key->value))]
                  ;(println (str "param-snippets-info: " param-snippets-info))
                  (substitute dsl param-snippets-info))))

(defn expand
  "Expand parameters inside a *DSL* QUERY."
  [query]
